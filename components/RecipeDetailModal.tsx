...
import type { RecipeStatus } from '@/lib/supabase-recipe-status';
...
export default function RecipeDetailModal({ open, onClose, recipe, recipeStatus, onDelete }: RecipeDetailModalProps) {
  ...
  // Get outfit count from status
  const outfitCount = recipeStatus?.outfitCount || 0;
  const linkedCount = recipeStatus?.linkedCount || 0;
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3, mb: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="h6" fontWeight={600}>
                {outfitCount > 0 ? `${outfitCount} (${linkedCount} linked)` : 'Not cooked yet'}
              </Typography>
            </Box>
          </Box>

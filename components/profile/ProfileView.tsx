...
export default function ProfileView({ profile }: ProfileViewProps) {
  ...
  const normalizedPillars = profile?.pillars ? normalizePillars(profile.pillars) : {};

  // Validate and log warnings
  useEffect(() => {
    if (profile?.pillars) {
      const validation = validatePillars(profile.pillars);
      if (!validation.isValid) {
        console.warn('⚠️ Non-canonical pillars detected in profile:', profile.customer_id);
        validation.warnings.forEach(warning => console.warn(`  - ${warning}`));
      }
    }
  }, [profile]);

  const topPillars = Object.entries(normalizedPillars)
      ...
      <div className="account-layout">
        ...
        <main className="account-content">
          <div className="customer-content view-content active" id="profileView">
            ...
            <div className="profile-section-animated">
              <StylePatterns pillars={normalizedPillars} />
            </div>

  ...
  try {
    ...
    for (let i = 0; i < outfitRows.length; i++) {
      ...
      try {
        ...
        // Tag with v2
        const tagResult = await tagOutfitV2(outfit, currentAttributes, {
...
async function findOutfitsWithProducts(
  ...
  while (matchingOutfits.length < limit && totalFetched < maxOutfitsToCheck) {
    ...
    if (excludeTagged) {
      query = query.or(`attributes->taggerVersion.is.null,attributes->taggerVersion.neq."v2"`);
    }
    ...
    const batchMatches = outfits.filter(outfit => {

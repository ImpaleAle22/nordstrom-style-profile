...
import { fetchOutfitRecipe } from './sanity-client';
import { generateOutfits } from './claude-api';
import { evaluateOutfit } from './scoring';
import { validateOutfitRecipe } from '../../recipe-builder/lib/outfit-building-rules';

/**
 * Validate outfit items against hard rules
 */
function validateOutfit(items: OutfitItem[]): { valid: boolean; errors: string[] } {
  const slots = items.map((item) => ({
    role: item.role,
  }));

  const result = validateOutfitRecipe({ slots });
 ...
 */
export async function composeOutfits(request: ComposeRequest): Promise<ComposeResponse> {
  ...
  try {
    ...
    const recipe = await fetchOutfitRecipe(request.recipeId);
    ...
    const generatedOutfits = await generateOutfits(recipe, request.customerSignals, maxOutfits);

    console.log(`\n🔍 Evaluating ${generatedOutfits.length} generated outfits...`);

    // 3. Validate and score each outfit
    const evaluatedOutfits: ComposedOutfit[] = [];
    let validCount = 0;
    let invalidCount = 0;
    ...
    for (let i = 0; i < generatedOutfits.length; i++) {
      ...
      console.log(
        `   ✅ Outfit ${i + 1}: ${evaluation.confidenceScore}/100 (${evaluation.poolTier}) - ` +
    ...
    const filteredOutfits = evaluatedOutfits.filter(
      (outfit) => outfit.confidenceScore >= minConfidence
    );

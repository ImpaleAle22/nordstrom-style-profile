...
import type { Product, OutfitRecipe, CustomerSignals, OutfitItem, SlotRole } from './types';
...
function buildOutfitPrompt(
  ...
  const slots = recipe.slots.map((slot, idx) => {
    ...
    const productList = products
      ...
      .map((p, i) =>
        `${i + 1}. ${p.brand} ${p.title} ($${p.price}) [ID: ${p._id}]\n` +
 ...
 */
interface ClaudeOutfitResponse {
  outfits: Array<{
...
function parseClaudeResponse(text: string): ClaudeOutfitResponse {
...
export async function generateOutfits(
  ...
  signals: CustomerSignals,
  maxOutfits: number = 5
): Promise<Array<{ items: OutfitItem[]; reasoning: string }>> {
  ...
  const outfits = parsed.outfits.slice(0, maxOutfits).map((outfit) => {
    const items: OutfitItem[] = outfit.items.map((item) => {
      ...
      const product = slot?.ingredientSet?.products.find(
        (p) => p._id === item.productId
      );

      if (!product) {
        throw new Error(
          `Product ${item.productId} not found in slot ${item.role}`
        );
      }

      return {
        role: item.role as SlotRole,
        product,
      };
    });

...
const STYLE_REGISTER_INDEX: Record<StyleRegister, number> = {
  'athletic': 0,
  'casual': 1,
  'smart-casual': 2,
  'business-casual': 3,
...
const STYLE_KEYWORDS: Record<StyleRegister, string[]> = {
  'athletic': [
 ...
 */
export function inferStyleRegister(product: Product): StyleRegister {
  const searchText = [
    product.title,
  ...
  const registerScores: Record<StyleRegister, number> = {
    'athletic': 0,
    'casual': 0,
    'smart-casual': 0,
    'business-casual': 0,
  ...
  for (const [register, keywords] of Object.entries(STYLE_KEYWORDS)) {
    for (const keyword of keywords) {
      if (searchText.includes(keyword)) {
        registerScores[register as StyleRegister] += 1;
      }
    }
  }

  // Return register with highest score, default to casual
  const topRegister = Object.entries(registerScores).reduce((a, b) =>
    a[1] > b[1] ? a : b
  )[0] as StyleRegister;
 ...
 */
export function scoreStyleRegisterCoherence(products: Product[]): number {
  ...
  if (spread === 3) return 40;
  if (spread >= 4) return 20;
 ...
 */
export function analyzeStyleRegisters(products: Product[]): {
  registers: Array<{ product: string; register: StyleRegister }>;

 ...
 */
export function getPrimaryColor(product: Product): string | null {
 ...
 */
export function getAllColors(product: Product): string[] {
 ...
 */
function isNeutralAnchor(colors: string[]): boolean {
 ...
 */
function isTonalNeutral(colors: string[]): boolean {
 ...
 * - Clashing: 20
 */
export function scoreColorHarmony(products: Product[]): number {
  ...
  if (isTonalNeutral(primaryColors)) return 95;
  if (isMonochromatic(primaryColors)) return 100;
  if (isNeutralAnchor(primaryColors)) return 90;
  ...
  if (neutralRatio >= 0.6) return 70; // mostly neutral with multiple accents
  if (neutralRatio >= 0.4) return 60; // mixed but not terrible
 ...
 */
export function analyzeColorHarmony(products: Product[]): {
  colors: Array<{ product: string; colors: string[] }>;
  harmonyScore: number;
  harmonyModel: string;
  assessment: string;
} {

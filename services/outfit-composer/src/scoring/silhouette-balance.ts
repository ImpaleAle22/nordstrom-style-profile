 ...
 */
export function inferSilhouette(product: Product): SilhouetteCategory {
  const searchText = [
    product.title,
  ...
  if (OVERSIZED_KEYWORDS.some((kw) => searchText.includes(kw))) return 'oversized';
  if (FITTED_KEYWORDS.some((kw) => searchText.includes(kw))) return 'fitted';
  if (FLOWY_KEYWORDS.some((kw) => searchText.includes(kw))) return 'flowy';
  if (STRUCTURED_KEYWORDS.some((kw) => searchText.includes(kw))) return 'structured';
  if (RELAXED_KEYWORDS.some((kw) => searchText.includes(kw))) return 'relaxed';
 ...
 */
function isSilhouetteBalanced(top: SilhouetteCategory, bottom: SilhouetteCategory): boolean {
  // Good pairings: opposite volumes
  if (top === 'oversized' && (bottom === 'fitted' || bottom === 'structured')) return true;
  if ((top === 'fitted' || top === 'structured') && (bottom === 'oversized' || bottom === 'flowy')) return true;

  // Good: structured + relaxed
  if (top === 'structured' && bottom === 'relaxed') return true;
  if (top === 'relaxed' && bottom === 'structured') return true;

  // Acceptable: both fitted or both relaxed
  if (top === 'fitted' && bottom === 'fitted') return true;
  if (top === 'relaxed' && bottom === 'relaxed') return true;

  // Bad: both oversized/voluminous
  if (top === 'oversized' && bottom === 'oversized') return false;
  if (top === 'flowy' && bottom === 'flowy') return false;

  // Unknown combinations: give benefit of doubt
  if (top === 'unknown' || bottom === 'unknown') return true;
 ...
 */
export function scoreSilhouetteBalance(products: Product[]): number {
  ...
  if (tops.length > 0 && bottoms.length > 0) {
    ...
    if (balanced) {
      ...
      if (
        (topSilhouette === 'oversized' && bottomSilhouette === 'fitted') ||
 ...
 */
export function analyzeSilhouetteBalance(products: Product[]): {
  silhouettes: Array<{ product: string; category: string; silhouette: SilhouetteCategory }>;
  balanceScore: number;
  assessment: string;
} {

...
exports.scoreOccasionAlignment = scoreOccasionAlignment;
exports.scoreSeasonFabricWeight = scoreSeasonFabricWeight;
exports.analyzeSeasonOccasion = analyzeSeasonOccasion;
// ============================================================================
// OCCASION ALIGNMENT (15% of confidence score)
// ============================================================================
const THEME_OCCASION_MAP = {
 ...
 */
function scoreOccasionAlignment(products, theme) {
    if (products.length === 0)
        return 0;
    const targetOccasions = THEME_OCCASION_MAP[theme.toLowerCase()] || [];
    if (targetOccasions.length === 0)
        return 75; // neutral when theme not recognized
    let matchCount = 0;
    let totalProducts = products.length;
    for (const product of products) {
        const productOccasions = product.occasions || [];
        const hasMatch = targetOccasions.some((target) => productOccasions.some((po) => po.toLowerCase().includes(target.toLowerCase())));
        if (hasMatch)
            matchCount++;
    }
    ...
    if (matchRatio >= 0.8)
        return 100; // 80%+ products match
    if (matchRatio >= 0.6)
        return 90; // 60-79% match
    if (matchRatio >= 0.4)
        return 75; // 40-59% match
    if (matchRatio >= 0.2)
        return 60; // 20-39% match
    return 40; // Less than 20% match
}
// ============================================================================
// SEASON / FABRIC WEIGHT APPROPRIATENESS (15% of confidence score)
// ============================================================================
const SEASON_FABRIC_GUIDE = {
...
const SEASON_MATERIAL_PREFERENCE = {
    spring: {
 ...
 */
function inferFabricWeight(product) {
    ...
    if (features.includes('heavyweight') || materials.includes('heavyweight'))
        return 'heavyweight';
    if (features.includes('lightweight') || materials.includes('lightweight'))
        return 'lightweight';
    if (features.includes('midweight') || materials.includes('midweight'))
        return 'midweight';
    ...
    if (materials.some((m) => heavyMaterials.includes(m)))
        return 'heavyweight';
    if (materials.some((m) => lightMaterials.includes(m)))
        return 'lightweight';
    if (materials.includes('cotton') || materials.includes('denim'))
        return 'midweight';
    // Check title for clues
    if (title.includes('puffer') || title.includes('insulated'))
        return 'heavyweight';
    if (title.includes('tank') || title.includes('sheer'))
        return 'lightweight';
    return null; // unknown
}
/**
 ...
 */
function scoreProductSeasonFit(product, season) {
    ...
    // Check materials
    const materials = product.materials?.map((m) => m.toLowerCase()) || [];
    const hasPreferred = materials.some((m) => materialPrefs.preferred.some((pref) => m.includes(pref)));
    const hasAvoided = materials.some((m) => materialPrefs.avoid.some((avoid) => m.includes(avoid)));
    if (hasPreferred)
        score += 20;
    if (hasAvoided)
        score -= 20;
 ...
 */
function scoreSeasonFabricWeight(products, signals) {
    if (products.length === 0)
        return 0;
    ...
    if (season === 'fall') {
        const hasOuterwear = products.some((p) => p.productType1 === 'Outerwear' || p.productType1 === 'Jacket/Sportcoat');
 ...
 */
function analyzeSeasonOccasion(products, theme, signals) {
    const season = signals.season === 'spring' ? 'spring' : 'fall';
...
//# sourceMappingURL=occasion-season.js.map